...
def build_ieee33(slack_vm_pu=1.03):
    ...
    bus_map = {int(bus): pp.create_bus(net, vn_kv=12.66, name=f'bus_{int(bus)}') for bus in buses['bus']}
    pp.create_ext_grid(net, bus_map[1], vm_pu=slack_vm_pu, name='substation_voltage_regulated')
    for _, row in branches.iterrows():
        pp.create_line_from_parameters(net, bus_map[int(row['from_bus'])], bus_map[int(row['to_bus'])], 1.0,
            float(row['r_ohm']), float(row['x_ohm']), 0.0, 0.4, name=f"{int(row['from_bus'])}-{int(row['to_bus'])}")
...
def ramp_limited_smooth(raw, max_ramp_mw=0.12):
    target = pd.Series(raw).rolling(3, min_periods=1, center=True).mean().to_numpy()
    smoothed = np.zeros_like(target, dtype=float)
    smoothed[0] = target[0]
    for i in range(1, len(target)):
        smoothed[i] = smoothed[i-1] + np.clip(target[i]-smoothed[i-1], -max_ramp_mw, max_ramp_mw)
    return smoothed
    ...
    for mode in ['no_ai', 'raw_ai', 'smoothed_ai']:
        for h, row in scenario.iterrows():
            ...
            try:
                ...
                undervoltage_buses = int((net.res_bus.vm_pu < 0.95).sum())
            ...
            records.append({
                'ai_bus': ai_bus_no, 'hour': int(h), 'mode': mode, 'converged': converged,
                'base_load_multiplier': multiplier,
                'ai_load_raw_mw': 0.0 if mode == 'no_ai' else float(raw[h]),
...
if __name__ == '__main__':
    ...
    all_results.to_csv(OUT / 'ieee33_ai_load_powerflow_results_moderate.csv', index=False, encoding='utf-8-sig')
    summary = all_results.groupby(['ai_bus','mode']).agg(
        ...
        ups_abs_comp_mwh=('battery_or_ups_compensation_mw', lambda x: float(np.abs(x).sum()))
    ).reset_index()
    summary.to_csv(OUT / 'ieee33_ai_load_powerflow_summary_moderate.csv', index=False, encoding='utf-8-sig')
